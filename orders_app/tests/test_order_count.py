...
class BaseOrderCountTestCase(APITestCase):
    """Shared test data for the order-count endpoint tests."""
...
class OrderCountSuccessTests(BaseOrderCountTestCase):
    """Test successful in-progress order count retrieval."""
    ...
    def test_counting_only_in_progress_orders(self):
        """Test that only in-progress orders contribute to the count."""

        self.assertEqual(self.response.data["order_count"], 1)


class OrderCountAuthenticationTests(BaseOrderCountTestCase):
    """Test that unauthenticated users cannot access the order count."""
